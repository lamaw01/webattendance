...
if($_SERVER['REQUEST_METHOD'] == 'POST' && array_key_exists('search_input', $input)){
    $search_input = $input['search_input'];
    $event_id = $input['event_id'];
    
    $concat_search_input = "%$search_input%";
    ...
    WHERE (tbl_event_log.employee_id LIKE :search_input OR tbl_employee.last_name LIKE :search_input OR tbl_employee.first_name LIKE :search_input) AND tbl_event_log.event_id = :event_id
    ORDER BY tbl_event_log.time_stamp DESC;";

    try {
        $get_sql= $conn->prepare($sql);
        $get_sql->bindParam(':search_input', $concat_search_input, PDO::PARAM_STR);
